...
decl_list
    ...
    | decl_list /*{S_beginScope(tab);}*/ func_defn /*{S_endScope(tab);}*/                            {$$ = $1; add_leaf($1, $2);}
    | decl_list func_decl                          {$$ = $1; add_leaf($1, $2);}
    | decl_list main_func                          {$$ = $1; add_leaf($1, $2);}
    | var_defn                                     {$$ = new_subtree(DECL_LIST_NODE, 1, $1);}
    | /*{S_beginScope(tab);}*/ func_defn /*{S_endScope(tab);}*/                                    {$$ = new_subtree(DECL_LIST_NODE, 1, $1);}
    | func_decl                                    {$$ = new_subtree(DECL_LIST_NODE, 1, $1);}
    | main_func                                    {$$ = new_subtree(DECL_LIST_NODE, 1, $1);}         
;

main_func
    : INT MAIN LPAREN RPAREN {S_beginScope(tab);} block {S_endScope(tab);}  {$$ = new_subtree(MAIN_NODE, 1, $6); setPos($$, 0);}
;
...
func_defn
    : func_header block                             {$$ = new_subtree(FUNC_DEFN_NODE, 2, $1, $2); S_endScope(p_tab); S_endScope(tab); }
;
func_decl
    : func_header SEMI                              {$$ = new_subtree(FUNC_DECL_NODE, 1, $1); S_endScope(p_tab); S_endScope(tab);}
;
stmt
    : assign_stmt                                  {$$ = $1;}
    | if_stmt                                      {$$ = $1;}
...
/*
func_out
    : func_call SEMI                               {$$ = new_subtree(FUNC_OUT_NODE, 1, $1);}
;*/
func_header
...
params
    : %empty                                      {S_beginScope(p_tab); S_beginScope(tab); $$ = new_subtree(PARAM_LIST_NODE,0);}
    | {S_beginScope(p_tab); S_beginScope(tab);} param_list                                  {$$ = $2;}
;
...
stmt_list
    ...
    | stmt_list {S_beginScope(tab);} block {S_endScope(tab);}                               {$$ = $1, add_leaf($1, $3);}
    | stmt                                         {$$ = new_subtree(BLOCK_NODE, 1, $1);}
    | func_decl                                    {$$ = new_subtree(BLOCK_NODE, 1, $1);}
    | {S_beginScope(tab);} block {S_endScope(tab);}{$$ = new_subtree(BLOCK_NODE, 1, $2);}
;
...
block_
    : {S_beginScope(tab);} block {S_endScope(tab);}             {$$ = new_subtree(BLOCK_NODE, 1, $2);}
    | {S_beginScope(tab);} stmt  {S_endScope(tab);}             {$$ = new_subtree(BLOCK_NODE, 1, $2);}
;
...
func_call
    : ID LPAREN arg_list RPAREN                    {$$ = new_subtree(FUNC_CALL_NODE, 2, $1, $3); setPos($$, tmp_cnt); tmp_cnt++;}
;
