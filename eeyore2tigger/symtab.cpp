...
map<string, bool> SymTable::parVar;
...
bool SymTable::IsPar(string varia)
{
    return parVar.find(varia) != parVar.end();
}

void SymTable::InsertPar(string varia)
{
    parVar[varia] = true;
}

void SymTable::initPar()
{
    parVar.erase(parVar.begin(), parVar.end());
}
